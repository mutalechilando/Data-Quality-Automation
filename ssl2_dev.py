...
#target_dataset_id = 'sXvRTmnS215'

#https://dhis2.moh.gov.zm/hmis-dev/api/29/dataSets/sXvRTmnS215

# Configuration for DHIS2 instances
source_dhis2_url = os.getenv('SOURCE_DHIS2_URL', 'https://lpho.more-zm.org:8445/api/29')
target_dhis2_url = os.getenv('TARGET_DHIS2_URL', 'https://dhis2.moh.gov.zm/hmis-dev/api/29/dataValueSets')
...
org_unit_mapping = load_mappings(file_path, sheet_name='org_units2').set_index('MORE-ZM')['HIA2'].to_dict()
...
for value in data_values_data.get('dataValues', []):
    ...
    mapping_rows = mappings_df[(mappings_df['morezm_dataid'] == morezm_dataid) & 
    ...
    if not mapping_rows.empty:
        for _, mapping_row in mapping_rows.iterrows():
            target_data_element = mapping_row['hia2_dataid']
            target_category_option_combo = mapping_row['hia2_categoryoptioncomboid']
